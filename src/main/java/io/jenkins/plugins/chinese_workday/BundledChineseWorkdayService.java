...
final class BundledChineseWorkdayService implements ChineseWorkdayService {
    ...
    private static final Map<Integer, HolidaySchedule> SCHEDULES = BundledHolidayCalendarLoader.load(SUPPORTED_YEARS);

    @Override
    public boolean isWorkday(LocalDate date) {
        HolidaySchedule schedule = SCHEDULES.get(date.getYear());
        if (schedule == null) {
            throw new IllegalArgumentException("No bundled Chinese holiday calendar is available for "
                    + date.getYear()
                    + ". Supported years: "
